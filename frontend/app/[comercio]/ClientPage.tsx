  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-sm flex-col items-center flex gap-6 z-10">
        ...
        {/* CATALOG / BOOKING HUB NAV BUTTON */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="w-full mt-2">
           <Link href={`/${commerceId}/catalogo`}>
              <div 
                 className="w-full p-5 rounded-3xl font-black text-center shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/20 hover:scale-[1.02] active:scale-95 transition-all flex justify-between items-center group"
                 style={{ backgroundColor: 'var(--theme)', color: getContrastYIQ(themeHex) }}
              >
                 <div className="flex items-center gap-3">
                    ...
                    <span className="text-lg">{data?.businessType === 'restaurante' ? 'Menú & Pedidos' : 'Catálogo & Agendamiento'}</span>
                 </div>
                 <div className="w-8 h-8 rounded-full flex items-center justify-center transition-colors" style={{ backgroundColor: getContrastYIQ(themeHex) === 'black' ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.2)', color: getContrastYIQ(themeHex) }}>
        ...
        {promos.map((promoId: string, idx: number) => (
          <motion.div key={idx} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + (idx * 0.1) }} className="w-full aspect-square mt-2 rounded-[2rem] overflow-hidden relative shadow-2xl ring-1 ring-white/10">
