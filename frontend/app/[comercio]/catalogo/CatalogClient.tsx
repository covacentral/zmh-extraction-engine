                ...
                if (isStoreMode && !forceList) {
                    return (
                       <div key={prod.id} onClick={() => router.push(`/${commerceId}/producto/${prod.id}`)} className={`bg-white/5 hover:bg-white/10 rounded-2xl overflow-hidden flex flex-col border border-white/5 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:-translate-y-1 ${isOut ? 'opacity-70' : ''}`}>
                          ...
                          <div className="p-3 flex flex-col flex-1 relative bg-gradient-to-t from-black/40 to-transparent">
                             <h3 className={`text-white text-xs font-bold leading-snug line-clamp-2 ${isOut ? 'line-through text-white/50' : ''}`}>{prod.name}</h3>
                             {refCode && <span className="inline-block mt-1 w-max text-white/40 text-[9px] font-mono uppercase tracking-wider">REF: {refCode}</span>}
                             
                             <div className="mt-auto pt-2">
                                <span className={`${isOut ? 'text-white/40' : 'text-[var(--theme)]'} font-black text-lg block drop-shadow-md`}>${price.toLocaleString('es-CO')}</span>
                                {(!isRestaurant && !asesorData && !vipClient && getProductPrice(prod, true) < getProductPrice(prod, false)) && (
                                   <span className="text-white/40 text-[9px] font-bold block leading-none mt-1">Mayorista: ${getProductPrice(prod, true).toLocaleString('es-CO')}</span>
                                )}
                             </div>
