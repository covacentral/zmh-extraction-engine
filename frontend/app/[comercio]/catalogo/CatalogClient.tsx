       ...
       <AnimatePresence>
        <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-[90%] max-w-sm">
           <button 
             ...
             className="w-full bg-[var(--theme)] text-white p-5 rounded-3xl font-black text-center shadow-[0_10px_30px_rgba(0,0,0,0.6)] border border-white/20 hover:scale-[1.02] active:scale-95 transition-all flex justify-between items-center group"
           >
              <div className="flex items-center gap-3">
                 ...
                 <span className="text-lg">Generar Ticket</span>
              </div>
              {cart.length > 0 && <span className="bg-white text-black px-3 py-1.5 rounded-full text-xs font-black shadow-inner shadow-black/20">${cartTotal.toLocaleString('es-CO')}</span>}
           </button>
        </motion.div>
      </AnimatePresence>
